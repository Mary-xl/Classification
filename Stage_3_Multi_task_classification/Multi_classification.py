...
import torch
from Project_I.Stage_3_Multi_task_classification.Multi_Network import *
...
def visualize_dataset(train_dataset, train_loader):
...
def train_model(model, criterion, optimizer, scheduler, num_epochs=50):
    ...
    best_acc={'overall':0.0, 'classes': 0.0, 'species':0.0}
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'val']:
            ...
            n=len(data_loaders[phase].dataset)
            m=data_loaders[phase]
            ...
            epoch_acc = 0.4*epoch_acc_classes+0.6*epoch_acc_species
            ...
            if phase == 'val' and epoch_acc > best_acc['overall']:
                ...
                best_acc ['overall'] = 0.4*epoch_acc_classes+0.6*epoch_acc_species
    ...
    print('Best val classes Acc: {:.2%}'.format(best_acc['overall']))
...
if __name__=='__main__':
    ...
    visualize_dataset(train_dataset, train_loader)

    network=multiNet().to(device)
    optimizer=optim.SGD(network.parameters(), lr=0.001, momentum=0.9)
    criterion=nn.CrossEntropyLoss()
    exp_lr_scheduler = lr_scheduler.StepLR(optimizer, step_size=1,
